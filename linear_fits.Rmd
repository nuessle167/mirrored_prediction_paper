...
ggplot(new_Goldstein_S2_Data[unknown_remover, ], aes(x = t90, y = Peak_E_over_Flu, colour = attr(predicted_svm, "probabilities")[,1])) +
  geom_point(size = 1)+ylab(expression(E[p]/S[prompt]~' 10kev-10MeV ('~eV ~ cm^{2} ~ erg^{-1} ~ ', GBM)'))+xlab('t90 (s, GBM)')+scale_colour_gradient2(
    low = "grey",
    mid =muted("blue"),
    high = "black",
    midpoint = 0.5
  )+geom_point(data = new_Goldstein_S2_mergers, color = "red")+geom_point(data = new_Goldstein_S2_collapsars, color = "cyan")+theme(legend.position="none")
plot(NULL ,xaxt='n',yaxt='n',bty='n',ylab='',xlab='', xlim=0:1, ylim=0:1)
legend("topleft", title='Burst Types', legend = c('Assigned Collapsar', 'Assigned Equally Likely', 'Assigned Merger', 'Detected Collapsar', 'Detected Merger'), col = c('black', muted('blue'), 'grey', 'cyan', 'red'),  pch = c(20, 20, 20, 20, 20))
