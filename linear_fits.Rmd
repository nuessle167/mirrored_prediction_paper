...
#        col = c('black', 'grey', 'red', 'blue'),
#        pch = c(20, 20, 4, 8))
low_error_data_long=data.frame(cbind(0, 10^newx, 0, 10^preds[ ,2], 0, 0))
colnames(low_error_data_long)<-colnames(Gehrels_S3_Data)
high_error_data_long=data.frame(cbind(0, 10^newx, 0, 10^preds[ ,3], 0, 0))
colnames(high_error_data_long)<-colnames(Gehrels_S3_Data)
low_error_data_short=data.frame(cbind(0, 10^newx2, 0, 10^preds2[ ,2], 0, 0))
colnames(low_error_data_short)<-colnames(Gehrels_S3_Data)
high_error_data_short=data.frame(cbind(0, 10^newx2, 0, 10^preds2[ ,3], 0, 0))
colnames(high_error_data_short)<-colnames(Gehrels_S3_Data)
# Gehrels_errorbar_data=data.frame(rbind(low_error_data_long, high_error_data_long, low_error_data_short, high_error_data_short))
# colnames(Gehrels_errorbar_data)<-c("Prompt_Flu", "X-ray_F11", "Group")
##
ggplot(Gehrels_S3_Long, aes(x = Prompt_Flu, y = `X-ray_F11`, colour=runif(1, 0, 1)), pch=20)+scale_colour_gradient2(low = "black",mid ="black", high = "black",  midpoint = 0.5, name="Burst Types")+geom_errorbar(aes(ymin=`X-ray_F11`-Flux_Err, ymax=`X-ray_F11`-Flux_Err))+geom_errorbar(aes(xmin=Prompt_Flu-Flu_Err, xmax=Prompt_Flu+Flu_Err))+ylab(expression('Afterglow '~Flux[11~Hr.]~' 0.3-10 keV (erg'~cm^{-2}~s^{-1}~', XRT)'))+xlab(expression('Prompt Fluence 10kev-10MeV (erg'~cm^{-2}~', GBM)'))+geom_point(size = 1)+scale_y_log10(labels = trans_format("log10", math_format(10^.x)))+scale_x_log10(labels = trans_format("log10", math_format(10^.x)))+geom_point(data = high_error_data_long, color = "black", pch=1)+geom_point(data = low_error_data_long, color = "black", pch=1)+geom_point(data = Gehrels_S3_Short, color = "grey", pch=20)+geom_point(data = high_error_data_short, color = "grey", pch=1)+geom_point(data = low_error_data_short, color = "grey", pch=1)+geom_point(data = Gehrels_S3_mergers, color = "red", pch=4)+geom_point(data = Gehrels_S3_collapsars, color = "blue", pch=8)#+geom_pointrange(data=Gehrels_S3_Short, aes(ymin=`X-ray_F11`-Flux_Err, ymax=`X-ray_F11`-Flux_Err), color = "grey", pch=20)+geom_pointrange(data=Gehrels_S3_Short, aes(ymin=`X-ray_F11`-Flux_Err, ymax=`X-ray_F11`-Flux_Err), color = "grey", pch=20)+geom_pointrange(data=Gehrels_S3_mergers, aes(ymin=`X-ray_F11`-Flux_Err, ymax=`X-ray_F11`-Flux_Err), color = "red", pch=4)+geom_pointrange(data=Gehrels_S3_mergers, aes(xmin=Prompt_Flu-Flu_Err, xmax=Prompt_Flu+Flu_Err), color = "red", pch=4)+geom_pointrange(data=Gehrels_S3_collapsars, aes(ymin=`X-ray_F11`-Flux_Err, ymax=`X-ray_F11`-Flux_Err), color = "blue", pch=8) +geom_pointrange(data=Gehrels_S3_collapsars, aes(xmin=Prompt_Flu-Flu_Err, xmax=Prompt_Flu+Flu_Err), color = "blue", pch=8)+geom_abline(intercept=modGehrels3$coefficients[1], slope=modGehrels3$coefficients[2], color='black', linetype="dashed")+geom_abline(intercept=modGehrels4$coefficients[1], slope=modGehrels4$coefficients[2], color='grey', linetype="dashed")
plot(NULL ,xaxt='n',yaxt='n',bty='n',ylab='',xlab='', xlim=0:1, ylim=0:1)
